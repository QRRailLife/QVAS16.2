...
function playAudio(audioPath) {
  ...
  // Normalize path slashes (convert backslashes to forward slashes)
  const normalizedPath = audioPath.replace(/\\/g, '/');
  
  // Determine fallback path for Form announcements or custom tnsFolder announcements
  let fallbackPath = null;
  if (normalizedPath.includes('/Form/')) {
    // Extract station name from Form path: Form/{FormCode}/{StationName}/{RouteLongName}.mp3
    const formPathMatch = normalizedPath.match(/\/Form\/[^\/]+\/([^\/]+)\/[^\/]+\.mp3$/);
    if (formPathMatch) {
  ...
  // For manually selected routes using any custom path (TNS_Special, Manual Route files, etc.), provide fallback to standard folders
  else if (normalizedPath.includes('QR_PIDS_AudioFiles/') && 
           (normalizedPath.includes('/TNS_Special/') || 
            normalizedPath.includes('/Manual Route files/') ||
            normalizedPath.includes('/PeakTimeExp/') ||
            normalizedPath.includes('/Form/'))) {
    // Extract filename from custom path to determine which fallback folder to use
    const filename = normalizedPath.substring(normalizedPath.lastIndexOf('/') + 1);
    
    if (filename.includes('TNS_')) {
      // TNS (Next Station) file format: TNS_{StationName}.mp3
      fallbackPath = `QR_PIDS_AudioFiles/TNS/${filename}`;
    } else if (filename.toLowerCase().includes(' mtg')) {
      // MTG (Mind The Gap) file format: {StationName} MTG.mp3
      fallbackPath = `QR_PIDS_AudioFiles/mind the gap/${filename}`;
    } else if (filename.toLowerCase().endsWith('.mp3') && !filename.includes('_') && !filename.includes('station')) {
      // NAA (Now Arriving At) file format: just station name (no prefix/suffix)
      fallbackPath = `QR_PIDS_AudioFiles/now arriving at/NAA ${filename}`;
    }
  }
  
  playAudioInternal(normalizedPath, fallbackPath);
}
...
function playAudioInternal(audioPath, fallbackPath) {
  ...
  try {
    ...
    audio.addEventListener('error', (e) => {
      ...
      if (fallbackPath && !audio.fallbackAttempted) {
      ...
      } else if (!audio.fallbackAttempted && (
        /(?:^|[\\/])TNS_Special[\\/]/i.test(audioPath) || 
        /(?:^|[\\/])Manual Route files[\\/]/i.test(audioPath) ||
        /(?:^|[\\/])PeakTimeExp[\\/]/i.test(audioPath)
      )) {
        // Auto-fallback: Custom folder file not found, try standard folder
        audio.fallbackAttempted = true;
        ...
        if (match) {
          ...
          if (filename.includes('TNS_')) {
            ...
            console.log(`   ➜ Custom path not found, falling back to TNS: ${tnsPath}`);
            playAudioInternal(tnsPath, null);
          } else if (filename.toLowerCase().includes(' mtg')) {
            // MTG file fallback
            const mtgPath = `QR_PIDS_AudioFiles/mind the gap/${filename}`;
            console.log(`   ➜ Custom path MTG not found, falling back to MTG: ${mtgPath}`);
            ...
            console.log(`   ➜ Custom path NAA not found, falling back to NAA: ${naaPath}`);
            playAudioInternal(naaPath, null);
          }
        }
      } else {
        // File not found and no fallback available - provide diagnostic info
        console.warn(`\n   📁 === FILE NOT FOUND - FALLBACK ALSO FAILED ===`);
        console.warn(`   Requested file: ${audioPath}`);
        console.warn(`   Expected location: QR_PIDS_AudioFiles/`);
        
        // Check if this is a custom path that should have had fallback
        const normalizedPath = audioPath.replace(/\\/g, '/').toLowerCase();
        if (normalizedPath.includes('/tns_special/') || normalizedPath.includes('/manual route files/') || normalizedPath.includes('/peaktimeexp/')) {
          console.warn(`\n   💡 This is a manual route using a custom folder.`);
          console.warn(`   📂 Both custom folder AND standard fallback were not found.`);
          console.warn(`   This means the file is missing from both locations.`);
          console.warn(`\n   To fix: Create the announcement file in one of these locations:`);
          
          if (normalizedPath.includes('tns_')) {
            console.warn(`      1. Add to custom folder: ${audioPath}`);
            console.warn(`      2. Or add to standard: QR_PIDS_AudioFiles/TNS/TNS_{StationName}.mp3`);
          } else if (normalizedPath.includes('mtg')) {
            console.warn(`      1. Add to custom folder: ${audioPath}`);
            console.warn(`      2. Or add to standard: QR_PIDS_AudioFiles/mind the gap/{StationName} MTG.mp3`);
          } else {
            console.warn(`      1. Add to custom folder: ${audioPath}`);
            console.warn(`      2. Or add to standard: QR_PIDS_AudioFiles/now arriving at/NAA {StationName}.mp3`);
          }
        } else if (audioPath.includes('/TNS/')) {
          console.warn(`\n   💡 This looks like a TNS (Next Station) file.`);
          console.warn(`   📂 Check if the file exists at:`);
          console.warn(`      • QR_PIDS_AudioFiles/TNS/TNS_{StationName}.mp3`);
...
async function loadRouteAnnouncements(formCode, longName) {
  // Browser/iPhone build: route-specific folder scanning requires Node/server APIs.
  // Standard bundled announcements remain available through getAnnouncementAudioPath().
...
const loadGTFSPatterns = async () => {
  try {
    console.log('🔄 Loading bundled GTFS patterns for browser/iPhone...');
    const response = await fetch('./2gtfs-patterns.json', { cache: 'no-store' });
    if (!response.ok) throw new Error(`2gtfs-patterns.json not found (${response.status})`);
    const data = await response.json();
    data.loadTime = new Date().toLocaleTimeString();
    console.log(`✅ Loaded bundled GTFS patterns: ${data.totalRoutes || 0} routes`);
    return data;
  } catch (error) {
    console.error('❌ Browser GTFS pattern load failed:', error);
    return null;
  }
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function selectRouteSelectionItem() {
    ...
    // Helper function to generate fallback paths for any manual route announcements
    // Always provide standard folder fallbacks for any custom tnsFolder
    const generateFallbackPaths = (station, tnsFolder) => {
      // If tnsFolder is specified (any custom folder), provide fallback to standard folders
      if (tnsFolder) {
        return {
          nextFallback: `QR_PIDS_AudioFiles/TNS/TNS_${station}.mp3`,
          arrivalFallback: `QR_PIDS_AudioFiles/now arriving at/NAA ${station}.mp3`,
          mtgFallback: `QR_PIDS_AudioFiles/mind the gap/${station} MTG.mp3`
        };
      }
      return { nextFallback: null, arrivalFallback: null, mtgFallback: null };
    };
    
    const pattern = stationEntries.map((station, index) => {
      const fallbacks = typeof station === 'string' ? generateFallbackPaths(station, manualRoute.tnsFolder) : {};
      return {
        ...
        announcements: typeof station === 'string' ? {
          next: { 
            audio: manualRoute.tnsFolder ? `${manualRoute.tnsFolder}/TNS_${station}.mp3` : '',
            fallback: fallbacks.nextFallback || ''
          },
          arrival: { 
            audio: manualRoute.tnsFolder ? `${manualRoute.tnsFolder}/${station}.mp3` : '',
            fallback: fallbacks.arrivalFallback || ''
          },
          mindTheGap: { 
            audio: manualRoute.tnsFolder ? `${manualRoute.tnsFolder}/${station} MTG.mp3` : '',
            fallback: fallbacks.mtgFallback || ''
          },
  ...
  function getPatternAnnouncementFallbackPath(stationRef, announcementType) {
    const stationObj = (typeof stationRef === 'object' && stationRef)
      ? stationRef
      : currentStations.find((station) => station && station.name === stationRef);
    ...
    if (announcementType === 'mindTheGap' || announcementType === 'MTG') {
      return stationObj.announcements.mindTheGap?.fallback || null;
    }

    if (announcementType === 'nextStation' || announcementType === 'next' || announcementType === 'TNS') {
      return stationObj.announcements.next?.fallback || null;
    }

    if (announcementType === 'arrival' || announcementType === 'nowArrivingAt' || announcementType === 'NAA') {
      return stationObj.announcements.arrival?.fallback || stationObj.announcements.next?.fallback || null;
    }

    if (announcementType === 'form') {
      return stationObj.announcements.form?.fallback || null;
    }
  ...
  function replayCurrentAnnouncement() {
    ...
    if (!displayWindow || displayWindow.closed) {
      displayWindow = window.open(`${window.location.origin}/display.html`, 'AnnouncementDisplay', 'width=900,height=200');
    }
  ...
  function doorsUnlock() {
    ...
    if (isAtFirstStation) {
      ...
      // Open display window if not already open or closed
      if (!displayWindow || displayWindow.closed) {
          displayWindow = window.open(`${window.location.origin}/display.html`, 'AnnouncementDisplay', 'AnnouncementDisplay', 'width=900,height=200');
      }
